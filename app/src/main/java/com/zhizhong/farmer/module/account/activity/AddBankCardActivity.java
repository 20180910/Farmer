...
public class AddBankCardActivity extends BaseActivity {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_add_bank_commit:
                ...
                if(TextUtils.isEmpty(userName)){
                    ...
                    showMsg("请选择银行");
                    return;
                }else if(!cb_bank_agree.isChecked()){
                    showMsg("请同意授权协议");
                    return;
                }
    ...
    private void getBankList() {
        showLoading();
        String rnd=getRnd();
        addSubscription(ApiRequest.getBankList(rnd,getSign("rnd",rnd)).subscribe(new MySub<List<BankObj>>(mContext) {
            @Override
            public void onMyNext(List<BankObj> list) {
                showBankDialog(list);
            }
        }));
    }
    ...
    private void addBank(String userName, String cardNum) {
        ...
        addSubscription(ApiRequest.addBank(map).subscribe(new MySub<BaseObj>(mContext) {
            @Override
            public void onMyNext(BaseObj obj) {
                showMsg(obj.getMsg());
                setResult(RESULT_OK);
